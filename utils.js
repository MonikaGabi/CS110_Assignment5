...
function matrixProduct(mat1, mat2)
{
    ...
    console.log("Matrix 1 with dimensions: ", dimensions_1);
    print_matrix(mat1)
    console.log("Matrix 2 with dimensions: ", dimensions_2);
    print_matrix(mat2)

    let new_matrix = empty_matrix(dimensions_1[0], dimensions_2[1]);
    
    console.log("Product matrix is");
    for (let i = 0; i < dimensions_1[0]; i++)
    {
        for (let k = 0; k < dimensions_2[1]; k++)
        {        
            new_matrix[i][k] = calculate_dot_product(mat1[i],get_column(mat2, k));
        }
    }
...
function sumOfMatrices(mat1, mat2) {
    ...
    //matrix addition is defined whenever their dimensions are equal, let's consider they are valid
    const dimensions_1 = [ mat1.length, mat1[0].length ];
    const dimensions_2 = [ mat2.length, mat2[0].length ];
